"""
Equity (cash segment) instrument handler.

Handles data fetching and preprocessing for individual stocks
traded on NSE/BSE.
"""
...
class EquityInstrument(BaseInstrument):
    """
    Handler for equity (cash) instruments.

    Characteristics:
        - Traded 09:15 – 15:30 IST
        - No expiry
        - Lot size typically 1 (cash) or per-share
        - Standard OHLCV data
    """

    instrument_type: str = "equity"
    description: str = "Individual equity / stock (cash segment)"

    def fetch_data(self, config: AppConfig, force_refresh: bool = False) -> pd.DataFrame:
        """Fetch equity OHLCV data via Kite Connect."""
        logger.info("Fetching equity data for %s", config.instrument.symbol)
    ...
    def preprocess(self, df: pd.DataFrame, config: AppConfig) -> pd.DataFrame:
        """Clean equity data (standard market hours filter, gap handling)."""
        return prepare_data(df)

    def compute_features(self, df: pd.DataFrame, config: AppConfig) -> pd.DataFrame:
        """Compute standard technical indicators for equity."""
